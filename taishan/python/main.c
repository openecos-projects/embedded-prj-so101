...
void home_with_monitor(void) {
    printf("\n>> Start Sync Home (Fast Mode)\n");
    // 6路同时起步
    for (int i = 1; i <= 6; i++) send_servo_cmd(i, arm_limits[i-1].home_pos, 0); 

    int all_done = 0, timeout = 0;
    uint8_t finished[6] = {0,0,0,0,0,0};
    while (all_done < 6 && timeout < 50) {
        all_done = 0;
        for (int i = 1; i <= 6; i++) {
            if (finished[i-1]) { all_done++; continue; }
            int16_t curr = read_servo_pos(i);
            if (curr != -1) {
                int target = (int)arm_limits[i-1].home_pos;
                int diff = (int)curr - target;
                // 整数换算 0.1 度
                int d_angle = (diff * 100 * arm_limits[i-1].dir) / 1138;
                printf("ID%d:%d.%d ", (int)i, (int)(d_angle/10), (int)MY_ABS(d_angle%10));
                if (MY_ABS(diff) < 8) { finished[i-1] = 1; printf("[OK] "); }
            }
        }
        printf("\n"); delay_ms(60); timeout++;
    }
